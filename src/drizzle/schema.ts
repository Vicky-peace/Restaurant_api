...
export const City = pgTable('city', {
    id: serial("id").primaryKey(),
    name: varchar("name", { length: 255 }).notNull(),
    state_id: integer("state_id").references(() => State.id, { onDelete: "cascade" }),
});
...
export const OrderMenuItem = pgTable('order_menu_item', {
    id: serial("id").primaryKey(),
    order_id: integer("order_id").references(() => Orders.id, { onDelete: "cascade" }),
    menu_item_id: integer("menu_item_id").references(() => MenuItem.id, { onDelete: "cascade" }),
...
export const OrderStatus = pgTable('order_status', {
    id: serial("id").primaryKey(),
    order_id: integer("order_id").references(() => Orders.id, { onDelete: "cascade" }),
    status_catalog_id: integer("status_catalog_id").references(() => StatusCatalog.id, { onDelete: "cascade" }),
    created_at: timestamp("created_at").defaultNow(),
});
...
export const Orders = pgTable('orders', {
    ...
    delivery_address_id: integer("delivery_address_id").references(() => Address.id, { onDelete: "cascade" }),
    user_id: integer("user_id").references(() => Users.id, { onDelete: "cascade" }),
    driver_id: integer("driver_id").references(() => Driver.id, { onDelete: "cascade" }),
    price: decimal("price").notNull(),
